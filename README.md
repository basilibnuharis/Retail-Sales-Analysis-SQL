...
-- Data Cleaning -- 
--Checking no.of NULL Value there in total?

 SELECT * FROM retail_sales
 WHERE 
	transactions_id IS NULL or Sale_date IS NULL or sale_time IS NULL 
	or gender IS NULLv or age IS NULL or category IS NULL or quantiy IS NULL
...
FROM (
    SELECT  
        YEAR(sale_date)  sales_year,
        MONTH(sale_date) sales_month,
...
2. **Set Up the Database**: Run the SQL scripts provided in the `Retail Sales Analytics project.sql` file to create and populate the database.
3. **Run the Queries**: Use the SQL queries provided in the `Retail Sales Analytics project.sql` file to perform your analysis.
